...
const Tela1 = ({ atualizarCurtidasVideos }) => {
  const [videos, setVideos] = useState(mockPrincipal);

  const cliqueCurtida = (videoId) => {
    const atualizarVideos = videos.map((video) =>
      video.id === videoId ? { ...video, liked: !video.liked } : video
    );

    setVideos(atualizarVideos);
    atualizarCurtidasVideos(atualizarVideos); 
  };
  ...
  return (
    <SafeAreaView style={estilos.container}>
      ...
      <Text style={estilos.titulo}>Vídeos</Text>
      <FlatList
        data={videos}
        renderItem={({ item }) => (
          <View style={estilos.videoContainer}>
            ...
            <View style={estilos.info}>
              ...
              <TouchableOpacity
                onPress={() => cliqueCurtida(item.id)}
                style={[
                  estilos.botaoCurtir,
                  { backgroundColor: item.liked ? '#FF0000' : '#eee' }, 
                ]}
              >
                <Text style={estilos.textoCurtir}>{item.liked ? '❤️ Curtido' : '🤍 Curtir'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
